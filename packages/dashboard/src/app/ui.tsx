...
/** The five gate outcomes, each mapped to a badge tone and a readable label. */
const GATE_OUTCOME_UI: Record<string, { cls: string; label: string }> = {
  meets_gate: { cls: "good", label: "meets gate" },
  fails_gate: { cls: "bad", label: "fails gate" },
  insufficient_data: { cls: "warn", label: "insufficient data" },
  judge_abstained: { cls: "warn", label: "judge abstained" },
  no_reliable_improvement: { cls: "", label: "no reliable improvement" },
};

export function gateBadge(outcome: string) {
  const ui = GATE_OUTCOME_UI[outcome] ?? { cls: "", label: outcome };
