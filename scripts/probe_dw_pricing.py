...
RATES = {"priority": (0.15, 0.003, 0.60), "flex": (0.12, 0.0024, 0.48)}
...
def model_row(snapshot):
    return next(
        (r for r in snapshot["by_model"] if r["model"] == MODEL),
...
def main():
    ...
    if not args.go:
        print(
            "Dry run: 4 calls, 64 output tokens each, $0.15 reservation cap; "
            "explicit 1h cache markers."
        )
        return
    ...
    try:
        ...
        for tier in ("priority", "flex"):
            ...
            for repeat in range(2):
                ...
                reserve = (len(wire) + 4096) * 0.30 / 1e6 + 64 * 0.60 / 1e6
                ...
                row["reconciled"] = (
                    delta == 1
                    and row["input_delta"] == raw["usage"]["prompt_tokens"]
